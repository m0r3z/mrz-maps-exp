# CLAUDE.md — mrz-maps-exp

Conventions et repères techniques pour Claude Code quand il opère sur ce plugin.

## Vue d'ensemble

**MRZ Maps Exp** est un plugin WordPress générique de cartographie Google Maps basé sur les champs ACF `google_map`. Multi-sites, minimaliste, configurable via l'admin, pensé pour être déployé tel quel sur plusieurs sites clients et personnalisé via le CSS du thème.

- Repo : https://github.com/m0r3z/mrz-maps-exp
- Branche principale : `main`
- Version courante : voir `define('MRZ_MAPS_EXP_VERSION', '...')` dans `mrz-maps-exp.php`

## Conventions de nommage

| Élément | Valeur |
|---|---|
| Slug plugin / text-domain | `mrz-maps-exp` |
| Nom affiché (Plugin Name) | `MRZ Maps Exp` |
...
```
mrz-maps-exp/
├── mrz-maps-exp.php                    # Bootstrap + autoloader PSR-4 maison
...
│   ├── js/mrz-maps-exp.js              # Init map, markers, filtres, search, popup, OMS, pagination
...
├── languages/
│   └── mrz-maps-exp.pot                # Généré via wp-cli i18n make-pot
...
- **Tooltip / popup** : classe `Popup` custom étendant `google.maps.OverlayView` (cf. `public/js/mrz-maps-exp.js`). 100% sous notre contrôle, surchargeable en CSS sans casser à chaque update Google.
...
1. Bump `define('MRZ_MAPS_EXP_VERSION', 'X.Y.Z')` + `Version: X.Y.Z` dans le header de `mrz-maps-exp.php`
2. Bump `Stable tag: X.Y.Z` + ajouter une entrée `= X.Y.Z =` dans le changelog de `readme.txt`
3. (Si chaînes traduisibles modifiées) regénérer le POT : `php /tmp/wp-cli.phar i18n make-pot . languages/mrz-maps-exp.pot --domain=mrz-maps-exp --package-name="MRZ Maps Exp"`
...
7. Zip propre : `cd wp-content/plugins && rm -f /tmp/mrz-maps-exp-X.Y.Z.zip && zip -rq /tmp/mrz-maps-exp-X.Y.Z.zip mrz-maps-exp -x "mrz-maps-exp/.git/*" "mrz-maps-exp/.gitignore" "mrz-maps-exp/.wordpress-org/*" "mrz-maps-exp/CLAUDE.md" "mrz-maps-exp/.DS_Store" "mrz-maps-exp/**/.DS_Store"`
...
- **Process release wp.org** (à scripter plus tard) :
  ...
  2. Checkout SVN : `svn co https://plugins.svn.wordpress.org/mrz-maps-exp /tmp/mrz-maps-exp-svn` (la première fois).
