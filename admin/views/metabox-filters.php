...
$modes_labels = array(
	'dropdown' => __( 'Menu déroulant', 'mrz-maps-experience' ),
	'radio'    => __( 'Boutons radio', 'mrz-maps-experience' ),
	'checkbox' => __( 'Cases à cocher', 'mrz-maps-experience' ),
);
$logic_labels = array(
	'or'  => __( 'OU', 'mrz-maps-experience' ),
	'and' => __( 'ET', 'mrz-maps-experience' ),
);

$acf_filters = (array) $values['acf_filters'];
?>

<h3 class="mrz-maps-exp-section-title"><?php esc_html_e( 'Recherche', 'mrz-maps-experience' ); ?></h3>
<p>
	<label>
		<input type="checkbox" name="mrz_maps_exp[search_enabled]" value="1" <?php checked( ! empty( $values['search_enabled'] ) ); ?> />
		<?php esc_html_e( 'Afficher un champ de recherche.', 'mrz-maps-experience' ); ?>
	</label>
</p>
<p>
	<label>
		<input type="checkbox" name="mrz_maps_exp[search_local_match]" value="1" <?php checked( ! empty( $values['search_local_match'] ) ); ?> />
		<?php esc_html_e( 'Suggérer aussi les posts dont le titre correspond à la saisie (en plus des adresses Google).', 'mrz-maps-experience' ); ?>
	</label>
</p>
<p>
	<?php esc_html_e( 'Position du champ :', 'mrz-maps-experience' ); ?>
	<?php
	$search_layouts_labels = array(
		'inline' => __( 'Dans le bloc filtres', 'mrz-maps-experience' ),
		'top'    => __( 'En haut, pleine largeur', 'mrz-maps-experience' ),
	);
	...
	<?php endforeach; ?>
	<br />
	<span class="description"><?php esc_html_e( 'En mode « En haut », le champ est sorti du bloc des filtres et occupe toute la largeur. Pratique quand les filtres sont placés sur le côté.', 'mrz-maps-experience' ); ?></span>
</p>
<p>
	<label>
		<?php esc_html_e( 'Rayon par défaut (km) :', 'mrz-maps-experience' ); ?>
		<input type="number" name="mrz_maps_exp[search_radius]" value="<?php echo esc_attr( $values['search_radius'] ); ?>" min="1" max="500" step="1" />
	</label>
	<span class="description"><?php esc_html_e( 'Appliqué uniquement quand l\'utilisateur sélectionne une adresse.', 'mrz-maps-experience' ); ?></span>
</p>
<p>
	<label>
		<?php esc_html_e( 'Libellé affiché au-dessus du champ :', 'mrz-maps-experience' ); ?>
		<input type="text" name="mrz_maps_exp[search_label]" value="<?php echo esc_attr( $values['search_label'] ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Rechercher', 'mrz-maps-experience' ); ?>" />
	</label>
</p>
<p>
	<label>
		<?php esc_html_e( 'Placeholder du champ :', 'mrz-maps-experience' ); ?>
		<input type="text" name="mrz_maps_exp[search_placeholder]" value="<?php echo esc_attr( $values['search_placeholder'] ); ?>" class="regular-text" placeholder="<?php esc_attr_e( 'Rechercher une adresse…', 'mrz-maps-experience' ); ?>" />
	</label>
</p>

<hr />

<h3 class="mrz-maps-exp-section-title"><?php esc_html_e( 'Options globales', 'mrz-maps-experience' ); ?></h3>
<p>
	<label>
		<input type="checkbox" name="mrz_maps_exp[show_filter_counts]" value="1" <?php checked( ! empty( $values['show_filter_counts'] ) ); ?> />
		<?php esc_html_e( 'Afficher le nombre de résultats à côté de chaque option de filtre.', 'mrz-maps-experience' ); ?>
	</label>
</p>
<p>
	<label>
		<input type="checkbox" name="mrz_maps_exp[url_filters_enabled]" value="1" <?php checked( ! empty( $values['url_filters_enabled'] ) ); ?> />
		<?php esc_html_e( 'Synchroniser les filtres avec l\'URL (lien partageable).', 'mrz-maps-experience' ); ?>
	</label>
	<br />
	<span class="description"><?php esc_html_e( 'Format : ?gm_ID_tax_slug=12,34&gm_ID_acf_field=valeur. L\'URL se met à jour automatiquement quand l\'utilisateur change un filtre.', 'mrz-maps-experience' ); ?></span>
</p>

<hr />

<h3 class="mrz-maps-exp-section-title"><?php esc_html_e( 'Filtres par taxonomie', 'mrz-maps-experience' ); ?></h3>
	...
	<?php foreach ( $all_tax as $tax ) : ?>
		...
		<div class="mrz-maps-exp-taxo-row" data-object-types="<?php echo esc_attr( $object_type ); ?>">
			<label class="mrz-maps-exp-taxo-col mrz-maps-exp-taxo-col-activate">
				<span><?php esc_html_e( 'Taxonomie', 'mrz-maps-experience' ); ?></span>
			...
			<label class="mrz-maps-exp-taxo-col">
				<span><?php esc_html_e( 'Libellé affiché', 'mrz-maps-experience' ); ?></span>
			...
			<label class="mrz-maps-exp-taxo-col">
				<span><?php esc_html_e( 'Type de filtre', 'mrz-maps-experience' ); ?></span>
			...
			<label class="mrz-maps-exp-taxo-col">
				<span><?php esc_html_e( 'Logique', 'mrz-maps-experience' ); ?></span>
				<select name="mrz_maps_exp[taxo_logic][<?php echo esc_attr( $slug ); ?>]" class="mrz-maps-exp-taxo-logic" title="<?php esc_attr_e( 'Combinaison entre cases cochées', 'mrz-maps-experience' ); ?>">
...
<p class="description"><?php esc_html_e( 'Seules les taxonomies liées au post type sélectionné sont affichées. Le libellé remplace le nom de la taxonomie affiché au-dessus du filtre sur le site.', 'mrz-maps-experience' ); ?></p>

<hr />

<h3 class="mrz-maps-exp-section-title"><?php esc_html_e( 'Filtres par champ ACF', 'mrz-maps-experience' ); ?></h3>
<p class="description">
	<?php esc_html_e( 'Pour les champs Select, Radio, Checkbox ou Vrai/Faux, les options sont détectées automatiquement depuis la configuration ACF. Pour les autres types (texte, nombre), les valeurs distinctes des posts sont collectées dynamiquement.', 'mrz-maps-experience' ); ?>
</p>
	...
	<?php foreach ( $acf_filters as $i => $row ) : ?>
		...
		<div class="mrz-maps-exp-acf-row" data-index="<?php echo (int) $i; ?>">
			<label class="mrz-maps-exp-acf-col">
				<span><?php esc_html_e( 'Nom du champ ACF', 'mrz-maps-experience' ); ?></span>
			...
			<label class="mrz-maps-exp-acf-col">
				<span><?php esc_html_e( 'Logique', 'mrz-maps-experience' ); ?></span>
				<select name="mrz_maps_exp[acf_filters][<?php echo (int) $i; ?>][logic]" title="<?php esc_attr_e( 'Combinaison entre cases cochées (sans effet en mode dropdown/radio)', 'mrz-maps-experience' ); ?>">
			...
			<button type="button" class="button mrz-maps-exp-acf-remove"><?php esc_html_e( 'Retirer', 'mrz-maps-experience' ); ?></button>
		</div>
	<?php endforeach; ?>
</div>

<p>
	<button type="button" class="button mrz-maps-exp-acf-add"><?php esc_html_e( 'Ajouter un filtre ACF', 'mrz-maps-experience' ); ?></button>
</p>
